...
class TextPredictionModel:
    ...
    def from_artefacts(cls, artefacts_path: str):
        ...
        # TODO: CODE HERE
        # load params
        params = json.load(open(os.path.join(artefacts_path, "params.json"), "r"))

        # TODO: CODE HERE
        # load labels_to_index
        labels_to_index = json.load(open(os.path.join(artefacts_path, "labels_index.json"), "r"))
    ...
    def predict(self, text_list, top_k=5):
        ...
        indexes = argsort(-preds)[:, :top_k]
        predictions = []
        for i in range(len(indexes)):
            predictions.append([self.labels_index_inv[index] for index in indexes[i]])
